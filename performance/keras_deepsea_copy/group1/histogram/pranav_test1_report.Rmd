...
for(i = unique(df$Annotation)){
    ggplot(df[df$Annotation == i,]) + geom_histogram(aes(x = log_odds)) + facet_grid(. ~ Annotation) + ggtitle(paste('Histogram of', i))
}
```
...
for(i = unique(df$Annotation)){
    ggplot(df[df$Annotation == i,]) + geom_histogram(aes(x = distance)) + facet_grid(. ~ Annotation) + ggtitle(paste('Histogram of', i))
}
```
